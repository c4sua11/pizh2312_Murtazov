...
int main() {
    ...
    // 3. Сортировка по убыванию с лямбда-функцией
    ...
    // 5. Поиск первого элемента > 2000 с лямбда-функцией
    ...
    // 6. Переместим элементы > 2000 в list с лямбда-функцией
    ...
    // 8. Сортировка по возрастанию с лямбда-функцией
    sort(payments.begin(), payments.end(),
        [](const shared_ptr<Payment>& a, const shared_ptr<Payment>& b) {
            return *a < *b;
        });
